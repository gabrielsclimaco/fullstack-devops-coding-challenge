...
locals {
  ...
  full_domain  = "${var.environment}.${local.base_domain}"
...
resource "aws_cloudwatch_log_delivery_source" "main" {
  ...
  name         = "${local.project_name}-logs-source"
...
resource "aws_cloudwatch_log_delivery_destination" "main" {
  ...
  name          = "${local.project_name}-logs-destination"
...
resource "aws_cloudwatch_log_delivery" "main" {
  ...
  s3_delivery_configuration {
    suffix_path = "/rdicidr-website/{DistributionId}/{yyyy}/{MM}/{dd}/{HH}"
  }
}
